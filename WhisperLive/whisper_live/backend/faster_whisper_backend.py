...
class ServeClientFasterWhisper(ServeClientBase):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            word_timestamps,
            local_agreement,
            local_agreement_window_seconds,
            local_agreement_hop_seconds,
            local_agreement_trailing_guard_seconds,
        ...
        try:
            if single_model:
                model_key = (self.model_size_or_path, device, self.compute_type)
                with ServeClientFasterWhisper.SINGLE_MODEL_LOCK:
                    if (
                        ServeClientFasterWhisper.SINGLE_MODEL is None
                        or ServeClientFasterWhisper.SINGLE_MODEL_KEY != model_key
                    ):
                        logging.info("Loading shared faster_whisper model for key=%s", model_key)
                        self.create_model(device)
                        ServeClientFasterWhisper.SINGLE_MODEL = self.transcriber
                        ServeClientFasterWhisper.SINGLE_MODEL_KEY = model_key
                    else:
                        logging.info("Reusing shared faster_whisper model for key=%s", model_key)
