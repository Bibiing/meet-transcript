"""
Penjadwal inferensi batch untuk WhisperLive.
Mengganti SINGLE_MODEL_LOCK per-sesi dengan sistem batch berbasis antrian.
Beberapa sesi mengirimkan audio ke antrian pusat; satu thread khusus mengumpulkan permintaan yang tertunda dan menjalankannya sebagai batch GPU melalui API encode() + generate() CTranslate2.
Untuk batch_size=1, kembali ke transcriber.transcribe() standar untuk perilaku identik dengan jalur non-batch.
Penggunaan:
    Aktifkan melalui flag CLI ``--batch_inference``. Pekerja batch dimulai secara malas setelah klien pertama terhubung dan model bersama dimuat.
Keamanan thread:
    - ``queue.Queue`` adalah thread-safe stdlib.
    - Setiap ``BatchRequest.future`` (``threading.Event``) ditulis oleh pekerja batch SEBELUM ``.set()``, dibaca oleh thread sesi SETELAH ``.wait()`` — tidak ada data race.
    - Hanya thread pekerja batch yang menyentuh model GPU — nol kontensi kunci antara thread sesi.
"""
    ...
    def _process_single(self, req: BatchRequest):
        ...
        try:
            result, info = self.transcriber.transcribe(
                ...
                hallucination_silence_threshold=req.hallucination_silence_threshold,
            )
